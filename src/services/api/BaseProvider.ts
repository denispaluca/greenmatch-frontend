...
export const request = async (resource: string, method: 'GET' | 'POST' | 'PATCH' | 'DELETE', body?: any) => {
  const cookie = getCookie();
  if (!cookie) {
    dispatch({ type: 'logout' });
  }

  const response = await fetch(`${process.env.PUBLIC_URL}/${resource}`, {
    method,
...
export const update = <T>(resource: string, id: string, body: object): Promise<T> => {
